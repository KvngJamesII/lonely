 ...
 * @param {string} quality - The quality being downloaded (SD or HD)
 */
function showDownloadModal(quality) {
    downloadStatus.textContent = `Preparing your ${quality} download...`;
...
/**
 * Completes the download progress animation and hides the modal
 * @param {number} intervalId - The interval ID to clear
 */
function completeDownload(intervalId) {
...
/**
 * Handles download errors by showing error message in modal
 * @param {number} intervalId - The interval ID to clear
 * @param {Error} error - The error that occurred
 * @param {HTMLElement} frameElement - The iframe to clean up
 */
function handleDownloadError(intervalId, error, frameElement) {
    // Clear the progress interval
    clearInterval(intervalId);
    
    // Update modal to show error
    downloadProgressBar.style.width = '100%';
    downloadProgressBar.style.backgroundColor = '#e74c3c';
    downloadStatus.textContent = 'Download failed. Trying alternative method...';
    
    // Log the error
    console.error('Download error:', error);
    
    // Remove the download frame
    ...
    setTimeout(() => {
        ...
        // Reset for next time
        downloadProgressBar.style.width = '0%';
        downloadProgressBar.style.backgroundColor = 'var(--color-primary)';
    }, 2000);
}
    ...
    if (data.video.downloads && data.video.downloads.length > 0) {
        ...
        if (sdQuality && sdQuality.downloadUrl) {
            ...
            downloadSD.onclick = function(e) {
                ...
                // Show download modal with progress bar
                const downloadInterval = showDownloadModal('SD');
                ...
                fetch(sdQuality.downloadUrl)
                    ...
                    .then(blob => {
                        ...
                        // Complete the download progress animation
                        completeDownload(downloadInterval);
                    ...
                    .catch(err => {
                        // Handle download error with our helper function
                        handleDownloadError(downloadInterval, err, downloadFrame);
        ...
        if (hdQuality && hdQuality.downloadUrl) {
            ...
            downloadHD.onclick = function(e) {
                ...
                // Show download modal with progress bar
                const downloadInterval = showDownloadModal('HD');
...
function isFacebookUrl(url) {
    ...
    const patterns = [
        ...
        /^(https?:\/\/)?(www\.|m\.|web\.|mbasic\.)?facebook\.com\/share\/r\/[\w-]+/i,
